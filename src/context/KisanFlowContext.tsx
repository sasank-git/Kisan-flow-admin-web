...
export const KisanFlowProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tokens' }, (payload: any) => {
        ...
        // Re-sync bookings locally
        loadSupabaseData();
        // 🔥 Trigger global UI update so Vercel graphs refresh instantly without page reload
  ...
  const bookNewSlot = (data: {
    ...
    // Background async insert to Supabase tokens table
  ...
  // 🔥 THE ASYNC DATABASE FIX FOR openExtraCounter
  const openExtraCounter = async (centreId: string) => {
    const targetCentre = centres.find(c => c.id === centreId || c.code === 'A102');
    if (!targetCentre) return;

    // Calculate the new counter total, capped at the building's max capacity
    const newActiveCounters = Math.min(targetCentre.totalCounters, targetCentre.activeCounters + 1);
    ...
    addNotification(
      `Counter ${newActiveCounters} Activated`,
      'Additional weighing & document counter opened to absorb peak arrivals.',
      'status'
    );

    // 2. Permanently save the newly opened counter to Supabase so it survives a page refresh
    try {
      const { error } = await supabase
        .from('centres')
        .update({ active_counters: newActiveCounters })
        .eq('id', centreId);

      if (error) {
        console.warn("Supabase Notice: Could not save active counters to DB:", error.message);
      }
    } catch (err) {
      console.warn("Error saving counter state:", err);
    }
  };
